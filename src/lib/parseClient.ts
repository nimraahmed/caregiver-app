...
export async function applyParse(text: string, update: (fn: (s: AppState) => AppState) => void): Promise<void> {
  ...
  update((s) => {
    if (!res || "error" in res || res.fallback) return { ...s, parsedText: text, plan: null };
    ...
    for (const [field, value] of Object.entries(res.fields) as [keyof ParseResult["fields"], unknown][]) {
      ...
      if (!stroke && ["weak_side", "grip_difficulty", "memory_or_attention_issues", "swallowing_issues", "new_stroke_signs"].includes(field)) continue;
